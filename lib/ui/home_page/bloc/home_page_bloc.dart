    ...
    on<HomePageEvent>((event, emit) async {
      await event.when(
        started: () async {
          ...
          emit(HomePageState.loaded(selectedDay: DateTime.now()));
        },
        addWeightRecord: (weightRecord) async =>
            await _addWeightRecord(weightRecord, emit),
        changeSelectedDay: (selectedDay) {
          emit(HomePageState.loaded(selectedDay: selectedDay));
        },
      );
    });
  }

  Future<void> _addWeightRecord(
    ...
    try {
      ...
      emit(HomePageState.success(message: 'Салмақ сәтті қосылды!'));
    } catch (e) {
      emit(HomePageState.failure(errorMessage: e.toString()));
    }
  }
}
